...
int app_main() {
    ...
    SEGGER_RTT_printf(0, "testing...\n");

    int ret = process_rgb_to_gray(dummy_hex, 1, 1, &gray_output);

    if (ret < 0) {
    	SEGGER_RTT_printf(0, "failed with error code: %d\n", ret);
    } else {
    	SEGGER_RTT_printf(0, "successful. processed %d pixel(s).\n", ret);
    	SEGGER_RTT_printf(0, "gray value of the pixel: %d\n", gray_output);
    }
